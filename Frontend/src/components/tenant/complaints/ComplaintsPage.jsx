                        ...
                        <Card key={req.id} onClick={() => navigate(`/${userName}/tenant/dashboard/complaints/${req.id}`)} className={`p-6 transition-all group hover:scale-[1.01] cursor-pointer`}>
                            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                                <div className="flex items-start gap-4">
                                    <div className={`p-3 rounded-lg border transition-colors duration-500 ${req.status === 'Resolved' ? (isDarkMode ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-emerald-100 text-emerald-700 border-emerald-200') : (isDarkMode ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-amber-100 text-amber-700 border-amber-200')} ${isDarkMode ? 'border-slate-700' : ''}`}>
