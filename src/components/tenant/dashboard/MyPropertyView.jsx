...
const MyPropertyView = ({
    ...
    const handleDownloadReceipt = async () => {
        setDownloading(true);
        try {
            const res = await fetch(`http://localhost:5000/api/payment/security-deposit/tenant/${user.id}`, {
                headers: {
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-baseline gap-1">
                                        <span className={`text-2xl font-black ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>₹{user.securityDeposit ? user.securityDeposit.toLocaleString() : "30,000"}</span>
