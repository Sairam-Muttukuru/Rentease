...
exports.createAndAddService = async (providerId, data) => {
    ...
    try {
        ...
        const svcRes = await client.query(
            `INSERT INTO services (type_id, name, description, base_price, image_url, features, provider_id)
             VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
            [finalTypeId, name, description, price, image_url, JSON.stringify(features || []), providerId]
        );
...
exports.updateService = async (id, providerId, data) => {
    ...
    try {
        ...
        // 2. Update global service info (name, description, image)
        await client.query(
            `UPDATE services 
             ...
             WHERE id = $4`,
            [name, description, image_url, serviceId]
        );

        // 3. Update provider-specific info (price, status)
        const res = await client.query(
        ...
        await client.query('COMMIT');
        return { ...res.rows[0], name, description, image_url };
    } catch (e) {
        await client.query('ROLLBACK');
        throw e;
    } finally {
...
exports.deleteService = async (id, providerId) => {
    await db.query("DELETE FROM provider_services WHERE id = $1 AND provider_id = $2", [id, providerId]);
};
...
exports.addCatalogService = async (data) => {
    ...
    const res = await db.query(
        "INSERT INTO services (type_id, name, description, base_price, image_url, features) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
        [type_id, name, description, base_price, image_url, JSON.stringify(features || [])]
    );
    return res.rows[0];
};
