...
const updateUser = async (id, data) => {
    ...
    values.push(id);
    const query = `UPDATE users SET ${fields.join(', ')} WHERE id=$${idx} RETURNING id, first_name, last_name, email`;

    return (await db.query(query, values)).rows[0];
};

module.exports = { createUser, findUserByEmail, updatePasswordByEmail, findUserById, getUserById, updateUser };
