...
async function testOverview() {
    const email = 'debug_admin_overview@rentease.com';
    ...
    try {
        ...
        if (res.ok) {
            ...
            console.log("Recent Activity Sample:", data.recent_activity?.[0]);
        } else {
            console.error(`❌ Overview failed: ${res.status} ${res.statusText}`);
